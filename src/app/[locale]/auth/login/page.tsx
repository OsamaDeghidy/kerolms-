...
import { ArrowLeft, ArrowRight, Lock, Mail, Loader2, Sparkles, Building2 } from "lucide-react";
      ...
      >
        ...
        <div className="relative bg-surface/30 backdrop-blur-[50px] p-12 lg:p-16 rounded-[3.5rem] border border-surface shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] border-b-4 border-b-primary/30 overflow-hidden">
          ...
          <div className="relative z-10 text-center mb-12 space-y-4">
            ...
            <p className="text-foreground-muted text-lg font-medium italic tracking-wide">{t('login_subtitle')}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="relative z-10 space-y-8">
            <div className="space-y-6">
              ...
              <div className="space-y-3 group">
                <div className="flex justify-between items-center ml-2">
                  <label className="text-xs font-black uppercase text-foreground-muted tracking-[0.3em] group-focus-within:text-primary transition-colors">
                     {t('password')}
                  </label>
                  <Link href="/auth/forgot-password" className="text-[10px] font-black uppercase tracking-widest text-primary hover:text-white transition-colors">{t('forgot_password')}</Link>
                </div>
                <div className="relative">
            ...
            >
              {isLoading ? (
                <><Loader2 className="animate-spin" /> {t('logging_in')}</>
              ) : (
                <>{t('login_button')} <Sparkles size={20} /></>
              )}
            </button>
            ...
            <div className="text-center pt-8 border-t border-surface/50">
              <p className="text-sm font-medium text-foreground-muted italic tracking-wide">
                {t('no_account')} 
                <Link href="/auth/register" className="text-primary font-black hover:text-white transition-colors ml-2 underline decoration-primary/30 underline-offset-4">
                   {t('register_now')}
                </Link>
              </p>
            </div>
